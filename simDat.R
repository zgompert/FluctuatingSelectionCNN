...
## write results
pmat<-matrix(rep(round(p,4),N),nrow=L,ncol=N,byrow=FALSE)
write.table(pmat,file="sim_p0.txt",row.names=FALSE,col.names=FALSE,quote=FALSE)

## write Ne, 100 samples from posterior, same beta for each
u<-600
l<-400
ne<-matrix(round(rbeta(100*N,10,10) * (u-l) + l,2),nrow=100,ncol=N)
write.table(ne,file="sim_ne.txt",row.names=FALSE,col.names=FALSE,quote=FALSE)

## random environment from standard normal
env<-matrix(round(rnorm((G-1) * N,0,1),3),nrow=G-1,ncol=N)
write.table(env,file="sim_env.txt",row.names=FALSE,col.names=FALSE,quote=FALSE)
